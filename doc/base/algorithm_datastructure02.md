...
4. gettop函数 若栈s不空, 则用e返回s的顶元, 并返回1, 否则返回0. 栈顶指针比顶元超前一个位置. 
5. push函数 将新元素e插入到栈s中. 栈顶指针比顶元超前一个位置. 
6. pop函数 若栈不空, 则删除栈s的顶元, 用e保存其值, 并返回成功标志1, 否则返回失败标志0. 
```
...
3. enque函数  插入新元素e, 使之成为队q队尾元素. 
4. deque函数  若队列非空, 则删除队q的队首元素. 并返回
```
