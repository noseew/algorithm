...
动态规划常规步骤思路, 专业思路
1. 定义状态, 状态: 该状态就是原问题, 同时也是子问题的解
- 如dp[n], dp[n-1]的含义, 
2. 设置初始条件, 边界
- 如dp[0]的值
3. 确定状态转移方程
- 如确定dp[n]和dp[n-1]的关系, 或者说dp[1]和dp[2]的关系, 详见找零钱


维基百科: 动态规划是解决复杂问题的一种方式, 将原问题分解成更简单的子问题, 且这些子问题只计算一次, 同时将计算的结果记录下来, 
从而来解决这个复杂的原问题

可解决的问题特点, 两个条件必须同时满足
1. 最优化原理, 最优子结构; 通过求解子问题的最优解, 可获得原问题的最优解
2. 无后效性; 
    某个阶段状态一旦确定, 则伺候过程中演变不在受此前各个状态即决策的影响, 
    在推导后面阶段状态的时候, 只关心前一个状态, 不关心更早的状态是怎么来的
    示例: a -> b -> c, a推导出b, b推导出c, 同时c不关心a是什么, 只关心b能推导出c
