...
public class Leetcode_11_maxArea {
    ...
    官方题解
    双指针, O(n)
        1. 首位指针, 连个指针分别指向头和尾, 然后计算他们的容量
        2. 关键: 首尾两个指针根据规则向中间移动
            1. 首尾两个指针总会有一个值比较短的, 将比较短的那个指针向中间移动
            2. 因为, 如果是值比较长的那个指针向中间移动的话, 那么新的容量肯定小于等于前一个的容量
            3. 而只有值较短的那个指针向中间移动, 才可能出现容量更大的指针配对情况
            4. 间每一次计算的容量保存下来, 取一个最大值, 就是当前数据中最大容量的值
     */
    /**
     * 官方题解
     *
